...
const crearResena = async (req, res) => {
  ...
  try {
    // 1. Buscar la reserva y validar pertenencia, sin reseña, y no eliminada
    ...
    if (!reserva) {
      return res.status(404).json({
        error: "Reserva no encontrada o no pertenece al usuario",
      });
    }

    // 2. Verificar si ya tiene reseña
    if (reserva.resena_id !== null) {
      return res.status(400).json({
        error: "Ya realizaste una reseña",
      });
    }

    // 3. Verificar si la fecha actual es igual o posterior a la fecha de inicio
    const hoy = new Date().toISOString().split("T")[0]; // formato YYYY-MM-DD
    if (reserva.fecha_inicio > hoy) {
      return res.status(400).json({
        error:
          "No se puede dejar una reseña antes de la fecha de inicio de la reserva",
      });
    }

    // 4. Verificar si el usuario está en lista negra
    ...
    // 6. Asociar la reseña a la reserva
    reserva.resena_id = nuevaResena.id;
    await reserva.save();
...
const obtenerResenasPorUsuario = async (req, res) => {
  ...
  try {
    ...
    if (resenas.length === 0) {
      return res
        .status(404)
        .json({ message: "No hay reseñas registradas para este usuario" });
    }
...
const obtenerTodasResenasPorUsuario = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Error al obtener todas las reseñas" });
  }
};
